...
- **Current theme:** `src/lib/activeTheme.ts` is a small external store (`useTheme()`, `useThemePref()`, `setTheme()`), applied on import so the page doesn't flash the default. The saved value is a `ThemePref`: a theme id or `system`, which `resolveThemePref` turns into `DEFAULT_THEME` (dark surroundings) or `DEFAULT_LIGHT_THEME` (light). Web: localStorage plus `prefers-color-scheme`. VS Code: the `jiraPlay.theme` setting; `boardHtml` writes the resolved theme onto `<html data-theme>` (plus `data-theme-pref="system"`) and the page then follows the `vscode-light`/`vscode-dark` classes on `<body>`.
- **Light themes** are `daylight`, `paper` and `office` (`LIGHT_THEMES`); both invert the slate scale, so new UI must read on them.
...
- **Command palette:** `CommandPalette` (⌘K / Ctrl+K) takes a `PalettePage` tree built in `App.tsx` (`buildPalette`, only while open); a command's `run` may return the next page for multi-step actions (mark done → which issue; assign → which issue → whom). Matching is `rankEntries` in `shared/palette.ts`.
- **Board density and stickiness:** `BoardToolbar`'s `density` (`comfortable` | `compact`) narrows `CharacterCard` (`compact` prop, in its memo comparator); density, grouping and sort persist through `readPref`/`writePref`.
- **Boss outcome:** `App.tsx` watches `bossState` per board and shows `BossOutcome` once per sprint (`jiraPlay.bossOutcome` pref): a victory when HP hits 0, a defeat when `sprintOver` with HP left. `lastBlow` (`shared/boss.ts`) credits the latest `resolvedAt`, so the credit survives a reload; `BossHit.by` names this session's hit. Both screens lead into `SprintRecap`.
- **Game guide:** `GameGuide` maps game terms to Jira terms; it opens once on the first real board (`jiraPlay.guideSeen`) and from the `?` button.
- **Profile history:** `Trends` also shows `personalBests` and `levelMilestones` (`shared/recap.ts`), and `AchievementBadges` has a `cabinet` mode listing every badge with its hint.
